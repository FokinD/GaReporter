		...
		public override void Execute(object parameter)
		{
			...
			try {
				foreach (var request in requests) {
					...
					if (ds.Tables.Contains(title) && ds.Tables[title].Columns.Contains(str_isoYearIsoWeek)) {//только если в измерениях есть неделя, остальные случаи необходимо доработать
						//Обрабатываем
